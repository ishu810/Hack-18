...
const generateItinerary = asyncHandler(async (req, res) => {
  ...
  const prompt = buildItineraryPrompt({
    ...
    selectedPlaces: orderedSelectedPlaces,
    budget: trip.budget,
    dates: trip.dates,
  ...
  try {
    ...
    console.log('⚠️ Itinerary path fallback: route-based generator used');

    const itineraryPlaces = orderedSelectedPlaces.slice(0, getTripDaysFromDates(trip.dates));

    const itineraryDays = await Promise.all(
      itineraryPlaces.map(async (place, index) => {
        const currentCity = place.location || trip.destination;
        const previousCity = index === 0
          ? trip.origin
          : (itineraryPlaces[index - 1]?.location || trip.origin);
        const isTransferDay = index > 0 && previousCity !== currentCity;

        const legRoute = isTransferDay
          ? await getGoogleRoute(previousCity, currentCity)
          : null;
